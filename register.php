...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        ...
        try {
            ...
            $student_id = filter_var(trim($_POST['student_id']), FILTER_SANITIZE_STRING);
            $contact_no = filter_var(trim($_POST['contact_no']), FILTER_SANITIZE_STRING);
            ...
            elseif (!preg_match("/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/", $password)) {
                ...
                if ($result->num_rows > 0) {
                    ...
                    try {
                        ...
                        $user_query = "INSERT INTO user (User_ID, Email, Name, Password, Contact_No, Login_Attempts, Account_Status) 
                                      VALUES (?, ?, ?, ?, ?, 0, 'active')";
                        ...
                        $notification_query = "INSERT INTO notifications (User_ID, Message) 
                                             VALUES (?, 'Welcome to BRAC University Lost & Found System!')";
