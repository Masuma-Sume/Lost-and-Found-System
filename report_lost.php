...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($item_name && $category_id && $location && $date_lost) {
        // Insert lost item
        $sql = "INSERT INTO items (User_ID, Item_Name, Item_Type, Category_ID, Location, Description, Color, Brand, Distinguishing_Features, Approximate_Value, Date_Lost, Date_Lost_Found, Status, Image_URL) VALUES (?, ?, 'lost', ?, ?, ?, ?, ?, ?, ?, ?, ?, 'open', ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('ssisssssssss', $user_id, $item_name, $category_id, $location, $description, $color, $brand, $distinguishing_features, $approximate_value, $date_lost, $date_lost, $image_url);
        if ($stmt->execute()) {
            ...
            if ($notification_sent) {
                $success_message = "Lost item reported successfully! All users have been notified.";
            } else {
                $success_message = "Lost item reported successfully! (Notification system temporarily unavailable)";
            }
        } else {
...
<body>
    ...
    <div class="container">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <h3 style="margin-top: 30px; color: #009688; border-bottom: 2px solid #009688; padding-bottom: 10px;">Verification Details</h3>
